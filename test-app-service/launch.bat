...
if "%*" == "" (
	...
	for %%s in ("-model" "-kjar" "test-app-service") do (
...
:startapp
	if not x%mavenInput:docker=%==x%mavenInput% (
		...
		call docker run -d -p 8090:8090 --name test-app-service apps/test-app-service:1.0-SNAPSHOT
	) else if not x%mavenInput:openshift=%==x%mavenInput% (
		echo Launching the application on OpenShift...
		call oc new-app test-app-service:1.0-SNAPSHOT
		call oc expose svc/test-app-service
	) else (
