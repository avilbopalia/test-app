...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "test-app-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name test-app-service apps/test-app-service:1.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app test-app-service:1.0-SNAPSHOT
    oc expose svc/test-app-service
else

	echo "Launching the application locally..."
	pattern="test-app-service"
	files=( $pattern )
	cd ${files[0]}
